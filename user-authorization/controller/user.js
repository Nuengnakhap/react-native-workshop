...
router.get("/", function (req, res, next) {
  res.send("respond with a user service");
});
  ...
  async (req, res) => {
    try {
      ...
      if (data) {
        ...
        res.status(404).json({ isError: true, errorMsg: "User not found" });
    } catch (err) {
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      if (find.dataValues.verifyToken != token)
        return res
          .status(404)
          .json({ isError: true, errorMsg: "Verify Token is invalid!" });
